...
SnRequest.prototype.exportUpdateSetToXml = async function (updateSetSysId, scope, dir) {
    var scriptStr = 'var current = new GlideRecord("sys_update_set");';
    scriptStr += 'current.get("' + updateSetSysId + '");';
    scriptStr += 'var updateSetExport = new UpdateSetExport();';
    scriptStr += 'var sysid = updateSetExport.exportUpdateSet(current);';
    scriptStr += 'gs.debug("@@" + sysid + "@@");';

    var resultStr = await this.evalScript(scriptStr, scope);
    var regex = /@@(.*)@@/gm;
    var exportUpdateSetSysId = regex.exec(resultStr)[1];

    var options = {
        'method': 'GET',
        'resolveWithFullResponse': true,
        'uri': this.uri + '/export_update_set.do?sysparm_sys_id=' + exportUpdateSetSysId + "&sysparm_delete_when_done=true"
    };
    var response = await this.executeRequest(options);
    var regPatt = /filename=(.*)/;
    var contentDisposition = response.headers['content-disposition'];
